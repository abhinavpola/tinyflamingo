...
class Tokenizer:
    ...
    def __init__(self, model_path: str, add_special_tokens: List[str] = []):
        ...
        for name, v in tensors.items():
            if "feed_forward" in name or "attention.w" in name:
                ...
                if isinstance(device, tuple):
                    ...
                    new_tensors[name.replace("weight", "scale")].shard_(
                        device, axis=None
                    )
            else:
                new_tensors[name] = v
    ...
    class _NF4Linear:
        def __init__(self, in_features, out_features, bias=False):
            ...
            self.weight = Tensor.empty(
                int(out_features * in_features / 2), dtype=dtypes.uint8
            )
            self.scale = Tensor.empty(
                int(out_features * in_features / block_size), 1, dtype=dtypes.float16
            )
        ...
        def quantize(state_dict: dict[str, Tensor], device) -> dict[str, Tensor]:
            ...
            for k, v in state_dict.items():
                if "feed_forward" in k or "attention.w" in k:
                    ...
                    new_state_dict[k.replace(".weight", ".scale")] = scale.cast(
                        dtypes.float16
                    )
                    if isinstance(device, tuple):
                        new_state_dict[k].shard_(device, axis=-1)
                        new_state_dict[k.replace("weight", "scale")].shard_(
                            device, axis=None
                        )
                else:
                    new_state_dict[k] = v
...
def build_transformer(model_path: Path, model_size="8B", quantize=None, device=None) -> Transformer:
    # build model
    if quantize == "int8":
        linear = Int8Linear
    elif quantize == "nf4":
        ...
        linear = nn.Linear
    model = Transformer(
        **MODEL_PARAMS[model_size]["args"], linear=linear, max_context=8192, jit=True
    )
