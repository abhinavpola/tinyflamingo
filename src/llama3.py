...
from .llama_deps import Transformer, convert_from_huggingface, convert_from_gguf, fix_bf16
...
class Tokenizer:
  ...
  def __init__(self, model_path: str):
    ...
    self.num_base_tokens = len(self.mergeable_ranks)
    self.special_tokens = [
      "<|begin_of_text|>",
      "<|end_of_text|>",
  ...
  def finish_init(self):
    self.special_tokens = {token: len(self.mergeable_ranks) + i for i, token in enumerate(self.special_tokens)}

    self.model = tiktoken.Encoding(name=self.model_path, pat_str=self.pat_str, mergeable_ranks=self.mergeable_ranks, special_tokens=self.special_tokens)
