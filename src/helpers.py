...
def count_nonzero(x, dim=None):
    return Tensor.sum(x.bool(), dim=dim)

# gated cross attention
class MaskedCrossAttention:
    def __init__(
        self,
        *,
        dim,
    ...
    def __call__(self, x, media, media_locations=None, use_cached_media=False):
        ...
        if exists(media_locations):
            media_time = Tensor.arange(T_img, device=x.device) + 1

            if use_cached_media:
                # text time is set to the last cached media location
                text_time = repeat(
                    count_nonzero(media_locations, dim=1),
                    "b -> b i",
                    i=T_txt,
                )
            else:
            ...
            mask_op = Tensor.eq if self.only_attend_immediate_media else Tensor.ge
